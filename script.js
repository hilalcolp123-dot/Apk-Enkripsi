...
// Caesar Cipher
// ======================
function caesar(text, shift, encrypt = true, caseMode = 'maintain', foreign = 'include') {
  let result = '';
...
const morseMapReverse = Object.fromEntries(Object.entries(morseMap).map(([k, v]) => [v, k]));
...
// Vigenère Cipher
// ======================
function vigenere(text, key, encrypt = true, caseMode = 'maintain', foreign = 'include') {
  let result = '';
...
function processCipher() {
  ...
  switch (cipherType) {
    case 'caesar':
      ...
      document.getElementById('alphabetBox').innerHTML = generateCaesarMap(shift);
      break;
    ...
    case 'vigenere':
      const vigenereKey = document.getElementById('vigenereKey').value.trim() || 'KUNCI';
      if (!vigenereKey) {
        showAlert('Masukkan kata kunci!', 'danger');
        return;
      }
      result = vigenere(inputText, vigenereKey, mode === 'encrypt', caseMode, foreign);
      break;
